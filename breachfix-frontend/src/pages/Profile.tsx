    ...
    return (
      <div className="min-h-screen bg-breachfix-navy flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-breachfix-white mb-4">Please log in to view your profile</h1>
          <p className="text-breachfix-gray">You need to be authenticated to access this page.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-breachfix-navy">
      <div className="container mx-auto px-4 py-8">
        {/* Profile Header */}
        <div className="bg-breachfix-dark rounded-lg p-8 mb-8">
          <div className="flex items-center space-x-6">
            ...
            />
            <div>
              <h1 className="text-3xl font-bold text-breachfix-white mb-2">{user?.username}</h1>
              <p className="text-breachfix-white mb-1">{user?.email}</p>
              <p className="text-breachfix-gray text-sm capitalize">Role: {user?.role}</p>
              {(user as any)?.bio && <p className="text-breachfix-white mt-2">{(user as any).bio}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="min-h-96">
          {activeTab === 'profile' && (
            <div className="bg-breachfix-dark rounded-lg p-8">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-breachfix-white">Profile Information</h2>
                <button
              ...
              {isEditing ? (
                <form onSubmit={handleEditSubmit} className="space-y-6">
                  <div>
                    <label className="block text-breachfix-white mb-2">Username</label>
                    <input
                      type="text"
                      name="username"
                  ...
                  <div>
                    <label className="block text-breachfix-white mb-2">Email</label>
                    <input
                      type="email"
                      name="email"
                  ...
                  <div>
                    <label className="block text-breachfix-white mb-2">Bio</label>
                    <textarea
                      name="bio"
                      value={editForm.bio}
                      onChange={handleInputChange}
                ...
                <div className="space-y-4">
                  <div>
                    <span className="text-breachfix-gray">Username:</span>
                    <p className="text-white">{user?.username}</p>
                  </div>
                  <div>
                    <span className="text-breachfix-gray">Email:</span>
                    <p className="text-white">{user?.email}</p>
                  </div>
                  <div>
                    <span className="text-breachfix-gray">Role:</span>
                    <p className="text-white capitalize">{user?.role}</p>
                  </div>
                  <div>
                    <span className="text-breachfix-gray">Member since:</span>
                    <p className="text-white">{new Date((user as any)?.createdAt || '').toLocaleDateString()}</p>
                  </div>
                  {(user as any)?.bio && (
                    <div>
                      <span className="text-breachfix-gray">Bio:</span>
          ...
          {activeTab === 'favorites' && (
            <div>
              <h2 className="text-2xl font-bold text-breachfix-white mb-6">My List</h2>
              {favoritesLoading ? (
                ...
                <div className="text-center py-12">
                  <div className="text-breachfix-gray text-xl mb-4">No favorites yet</div>
          ...
          {activeTab === 'history' && (
            <div>
              <h2 className="text-2xl font-bold text-breachfix-white mb-6">Watch History</h2>
              {historyLoading ? (
                <div className="flex justify-center py-12">
                  <motion.div
                    animate={{ rotate: 360 }}
                ...
                <div className="space-y-4">
                  {watchHistory.map((item) => (
                    <div key={item._id} className="bg-breachfix-dark rounded-lg p-4 flex items-center space-x-4">
                      <img
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-breachfix-gray text-sm capitalize">{item.contentType}</p>
                        ...
                        <span className="text-breachfix-gray text-xs">{item.progress}% watched</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-12">
                  <div className="text-breachfix-gray text-xl mb-4">No watch history</div>
          ...
          {activeTab === 'settings' && (
            <div className="bg-breachfix-dark rounded-lg p-8">
              <h2 className="text-2xl font-bold text-breachfix-white mb-6">Account Settings</h2>
              
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold text-breachfix-white mb-4">Notifications</h3>
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 text-breachfix-white">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-breachfix-white mb-4">Privacy</h3>
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 text-breachfix-white">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-breachfix-white mb-4">Security</h3>
                  <div className="space-y-3">
