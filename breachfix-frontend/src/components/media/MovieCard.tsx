      ...
      <Link to={`${getCorrectRoute()}/${movieId}`} onClick={handleMovieClick}>
        <div className="relative overflow-hidden rounded-lg shadow-lg">
          ...
          {movie.episodeNumber && movie.seasonNumber && (
            <div className="absolute top-2 right-2 bg-breachfix-emerald text-breachfix-white px-2 py-1 rounded-full text-xs font-semibold z-10">
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                {movie.genres && Array.isArray(movie.genres) && movie.genres.length > 0 ? (
                  movie.genres.slice(0, 2).map((genre, index) => (
                    <span
                      key={index}
                      className="text-xs bg-breachfix-gold text-breachfix-navy px-2 py-1 rounded"
                    >
                      {genre}
                    </span>
                  ))
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="bg-breachfix-gold text-breachfix-navy p-3 rounded-full shadow-lg">
